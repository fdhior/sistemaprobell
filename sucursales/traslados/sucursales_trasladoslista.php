...
<?php
        ...
        if (!isset($error)) {
                ...
                } // Cierre de if filtfecha
        } // Cierre de if isset error

        if (!isset($error)) {
                // Definicion de los parametros de la consulta
                //                    0         1          2         3          4
                $cols_arr = array("idtras", "archivo", "tamano", "origen", "fechaenvio");
...
// Se determina que buscar modificando la variable where_clausa
        ...
        // Patr&oacute;n reutilizable de busqueda
                $buscpattern1 = "(sucr_tlog.idinmu = '$userinmuid' AND sucr_tlog.descargado = 0) OR (sucr_tlog.archivo LIKE '%AD0%' AND sucr_tlog.descargado = 0)";
...
<script>
    function abreventana<?php  echo $i; ?>(){
        ...
        <tr class="<?php
                           ...
                           ?>">
     <form action="<?php echo $target_link; ?>" method="post" enctype="multipart/form-data" name="form<?php echo $i; ?>" id="form<?php echo "$i"; ?>">
       ...
       <td class="traslst_td4"><?php
                                   ...
                                   echo $dest_parse; // Destino ?></span></td>
      <td class="traslst_td5"><?php echo $row[4]; // Fecha y Hora de Envio ?></span></td>
      <td class="traslst_td6"><input type="hidden" name="idtras" id="idtras" value="<?php echo $row[0]; ?>" />
<!--    <input type="hidden" name="file" id="file" value="<?php // echo "$row[1]"; ?>" /> -->
        ...
        <input class="letra_boton11" type="submit" name="button" id="button" value="Descargar Traslado" onclick="javascript: abreventana<?php echo "$i"; ?>()" /></td>
     </form>
     <td class="traslst_td7">&nbsp;</td>
</tr>
<?php
                $i++;
				$j++;
...
<?php
		...
		if (!isset($error) and isset($_POST['busca']) and $mquery_nrows < 1) {
			...
			if (!isset($error) and !isset ($_POST['busca']) and $mquery_nrows < 1) {
				echo '<div id="apDiv2"';
				echo '<span class="letra_predeterminada">Por el momento no hay traslados que descargar</span>';
				echo '</div>';
			}
		}
