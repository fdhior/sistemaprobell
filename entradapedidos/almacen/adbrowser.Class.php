    ...
    public function deleteFiles($aFiles){
		...
		include $rutafunciones.'consultas.php';
            ...
            foreach ($aFiles as $aFilesNames){
                if (is_dir($dir.$aFilesNames)) {
                    ...
                    if (@unlink($dir.$aFilesNames)) {
                        ...
                        $this->sMessage = "Los Archivos de Pedido fueron Eliminados";
                    } else {
            ...
            $i++;
                ...
                // ALTER TABLE `sucr_plog` MODIFY COLUMN `idtras` INTEGER UNSIGNED DEFAULT NULL, DROP PRIMARY KEY;
                $aff_table = "sucr_plog";
                $action = "MODIFY COLUMN `idped`";
                $modifiers = "INTEGER UNSIGNED DEFAULT NULL, DROP PRIMARY KEY";

                $result = gnrl_altertable_query($aff_table, $action, $modifiers);       
                
                // ALTER TABLE `sucr_plog` AUTO_INCREMENT = 0;
                $action = "AUTO_INCREMENT = 0";
                $modifiers = "";

                $result = gnrl_altertable_query($aff_table, $action, $modifiers);       

                // ALTER TABLE `sucr_plog` ADD COLUMN `idpedtemp` INTEGER UNSIGNED NOT NULL DEFAULT NULL AUTO_INCREMENT AFTER `idped`, ADD PRIMARY KEY (`idpedtemp`);
                $action = "ADD COLUMN `idpedtemp`";
                $modifiers = "INTEGER UNSIGNED NOT NULL DEFAULT NULL AUTO_INCREMENT AFTER `idped`, ADD PRIMARY KEY (`idpedtemp`)";

                $result = gnrl_altertable_query($aff_table, $action, $modifiers);       

                // ALTER TABLE `sucr_plog` DROP COLUMN `idped`;
                $action = "DROP COLUMN `idped`";
                $modifiers = "";

                $result = gnrl_altertable_query($aff_table, $action, $modifiers);       

                // ALTER TABLE sucr_plog CHANGE idpedtemp idped INTEGER UNSIGNED AUTO_INCREMENT;                
                $action = "CHANGE idpedtemp idped";
