...
<body <?php if ($_POST['btExportar']) { echo 'onload="enviaDatosReporte()"'; } ?>>
...
/* ---------------------------------------- TERMINAVALIDACION DE DATOS ---------------------------- */
     ...
     if (!isset($error) and isset($quien_busc)) {
		...
		$cols_arr      = array("gnrl_echk.idempleado",        // 0
							   ...
							   "gnrl_echk.ipRemota",          // 10   
							   "gnrl_echk.hostnameRemoto",    // 11 
							   "gnrl_echk.nombreHosts");      // 12  
			$num_cols      = count($cols_arr);
		   	$join_tables   = '1';
		    $tables_arr    = array("gnrl_echk",
								   "gnrl_empl",
			...
			$connect       = '1';
			$order         = "idfis, gnrl_echk.checada_fecha, gnrl_echk.checada_hora";
			$dir	       = "ASC";
//      $connect = '0';
...
?>
	...
	<tr id="<?php echo $row[0]; ?>" class="<?php $oddevencheck = $i % 2;  
        ...
        <td class="table_td5"><?php 
			...
			echo $row[6].'/'.$idinmu_chk[0];							       // Lugar Trabajo/Lugar Checada   ?></td> 
        <td class="table_td6"><?php echo $row[7].' '.$row[8];                  // Fecha/Hora                    ?></td>
        <td class="table_td7"><?php echo 'IP Remota: <strong>'.$row[10].'</strong>
							  <br />Hostname Remoto: <strong>'.$row[11].'</strong>
                             <br />Valor Hosts: <strong>'.$row[12].'</strong>';// IP Remota/Hotsname/Hosts      ?></td>
...
<form action="<?php echo $target_link; ?>" method="post" name="form1" id="form1" target="_self">
 
  <input type="hidden" name="formato" id="sel_formato" value="excel" />
