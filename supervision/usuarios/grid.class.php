...
{      
  ...
  // update a product
  public function updateRecord($id, $contra, $sel_usrstatus, $usertypepass)
  {
...
//    $correoe        = $this->mMysqli->real_escape_string($correoe);
    $sel_usrstatus  = $this->mMysqli->real_escape_string($sel_usrstatus);
    $usertypepass   = $this->mMysqli->real_escape_string($usertypepass);
	...
	if ($contra <> '') {
		...
		$colsvalarr[] = "pswd = '$md5_contra'";
		if ($usertypepass == 1) {
			$colsvalarr[] = "textpass = 'oculta'";
		} else {
			$colsvalarr[] = "textpass = '$contra'";
		}
	}
	
/*	if ($correoe <> 'No Registrado') {
		$colsvalarr[] = "correoe = '$correoe'";
	}
*/
	if ($sel_usrstatus <> 0) {
		$colsvalarr[] = "idusrstatus = '$sel_usrstatus'"; 
	}
						
	if (isset($colsvalarr)) {
		$numcols = count($colsvalarr);
		$where_clause = "iduser = '$id'";
